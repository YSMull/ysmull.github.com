...
由于 HashSet 其实是把 element 存储在 HashMap 的 key 上，*HashMap#getNode* 根据 key 来找结点，如果不为 `null`，则证明元素存在于这个 Map，也就是存在于这个 HashSet。所以对两个 HashMap 进行 equals 操作时，被包装类型必须重写 `equals()` 和 `hashCode()` 方法，否则会导致
